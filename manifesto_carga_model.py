...
class ManifestoCarga(db.Model):
    ...
    informacoes_motorista = db.Column(db.String, nullable=False)
    informacoes_cliente = db.Column(db.String, nullable=False)
    ...
    distancia = db.Column(db.Float, nullalbe=False) # para o calculo do frete
...
def create_carga(carga):
    nova_carga = ManifestoCarga(
        ...
        informacoes_cliente = carga["innformacoes_cliente"],
    ...
    db.session.commit()
    return {"message":"Nova carga adicionada com sucesso"} 
...
def read_todas_cargas():
    ...
    print(cargas)
    return [cargas.to_dict() for carga in cargas]


def read_alunos_id(id_carga):
...
def update_carga(id_carga, dados_atualizados):
    ...
    carga.informacoes_cliente = dados_atualizados["innformacoes_cliente"]
...
def delete_carga_id(id_carga):
    ...
    db.session.commit()
    return {"message":"Carga deletada com sucesso!"}
...
def delete_todas_cargas():
    ...
    for carga in cargas:
        db.session.delete(carga)
    db.session.commit()
    return {'message':"Cargas deletadas com sucesso!"}